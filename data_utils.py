...
def get_annotated_acos_targets(sents, labels, type = 'rest'):
    """
    思路：将句子改造成 [aspect|category1|op1, op2, ...|st; category2|op1, op2, ...|st]
        1_先构造一个字典
        2_再完成 annotate
    """
    ...
    def judge_sentiment(st1, st2):
        """
        思路：一个有情感，一个中立 -> 结果为有情感。其余情况，相同返回，不同返回 conflict
        """
        if st1 == st2:
            return st1
        else:
    ...
    for i in range(num_sents):
        # 1_存字典
        tuples = labels[i]
        its = {}                    # 辅助构造 annotated 字典,{aspect:{category1:[st, [op1, op2, op3]], category2:[st, [op1,op2,...]]}}
        # tup: ([at_begin, at_end], 'Entity#Attribute', [st], [op_begin, op_end])
        for tup in tuples:
            ap, ct, st, op = tup[0], tup[1], tup[2], tup[3]
            if ap[-1] != -1:                # 数据中是左闭右开，改成左闭右闭
                ap[-1] = ap[-1] - 1
            ap = tuple(ap)                  # 因为 ap 要作为 dict 的索引，而 list 不可作为索引    
            ct = ct.strip()
            ...
            # 如果存在 ap
            if tempdict:
                templist = tempdict.get(ct)     # templist[0] 是 sentiment
                # 如果存在 ct（ct 相同）
                if templist:
